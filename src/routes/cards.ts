...
router.post('/text-import', (req: Request, res: Response) => {
  const db = getDb();
  const { text = '', brand } = req.body as { text?: string; brand?: string };
...
router.get('/stats', (_req: Request, res: Response) => {
  ...
  const active = (db.prepare("SELECT COUNT(*) as c FROM cards WHERE deleted = 0 AND status = 'active' AND allocatedTo IS NULL").get() as any).c;
  ...
  const brands = db.prepare(`
    SELECT brand, COUNT(*) as total,
      SUM(CASE WHEN status = 'active' AND allocatedTo IS NULL THEN 1 ELSE 0 END) as active
